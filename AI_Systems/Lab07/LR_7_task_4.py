...
def plot_clusters(X, labels, names, exemplars, out_path="affinity_clusters.png"):
    ...
    for i, nm in enumerate(names):
        plt.text(Z[i,0]+0.02, Z[i,1]+0.02, nm.split()[0], fontsize=9)  # короткий підпис
...
def print_cluster_members(labels, tickers, names, exemplars):
    ...
    for lab in sorted(clusters):
        star = " *" if exemplars is not None and lab in range(len(exemplars)) else ""
        print(f"\nКластер {lab}{star}:")
        for t, n in clusters[lab]:
            print(f"  {t:6s}  —  {n}")

def main():
    ap = argparse.ArgumentParser(description="LR_7_task_4: AffinityPropagation for stock clusters")
    ...
    plot_clusters(X, labels, names, exemplars, out_path="affinity_clusters.png")
